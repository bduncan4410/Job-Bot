...
def validate_yaml():
    parameters = open_yaml()
    mandatory_params = ['emailLink', 'passwordLink', 'emailDice', 'passwordDice', 'disableAntiLock', 'remote', 'experienceLevel', 'jobTypes', 'dateLink','dateDice' ,
    ...
    assert validate_email(parameters['emailLink'])
    assert len(str(parameters['passwordLink'])) > 0

    assert validate_email(parameters['emailDice'])
    assert len(str(parameters['passwordLink'])) > 0
    ...
    assert len(parameters['dateLink']) > 0
    dateLink = parameters.get('dateLink', [])
    at_least_one_date = False
    for key in dateLink.keys():
        if dateLink[key]:
            at_least_one_date = True
    assert at_least_one_date

    assert len(parameters['dateDice']) > 0
    dateDice = parameters.get('dateDice', [])
    at_least_one_date = False
    for key in dateDice.keys():
        if dateDice[key]:
            at_least_one_date = True
    assert at_least_one_date
